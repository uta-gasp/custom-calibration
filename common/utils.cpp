...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
int Utils::sign(double a) {	return a < 0.0 ? -1 : (a > 0.0 ? 1 : 0); }
int Utils::sign(float a) {	return a < 0.0f ? -1 : (a > 0.0f ? 1 : 0); }
int Utils::sign(int a) {	return a < 0 ? -1 : (a > 0 ? 1 : 0); }
int Utils::sign(short a) {	return a < 0 ? -1 : (a > 0 ? 1 : 0); }

//---------------------------------------------------------------------------
bool Utils::sameSign(double a, double b)
{
	return sign(a) == sign(b);
}

//---------------------------------------------------------------------------
int Utils::randInRange(int _min, int _max)
{
	return random(_max - _min + 1) + _min;
}

//---------------------------------------------------------------------------
bool Utils::loadBitmapFromPNG(UINT resourceID, Gdiplus::Bitmap** bitmapOut)
{
	bool bRet = false;
...
TStrings* TiStrings::split(const String& aText, const String& aSeparator)
{
	...
	strcpy(buffer, aText.c_str());

	char* sep = aSeparator.c_str();
