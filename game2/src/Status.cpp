		...
		TiScene(aManager, aScreenSize, aViewport)
{
	...
	btnButtonContinue = new TiAnimation(false);
	btnButtonContinue->addFrames(IDR_STATUS_BUTTON_CONTINUE, 209, 70);
	btnButtonContinue->placeTo(Offset.x + aViewport.Width/4*3, Offset.y + aViewport.Height/3*2);
	aManager->add(btnButtonContinue);

	iButtonContinue = TiRect(btnButtonContinue->X - btnButtonContinue->Width / 2,
			btnButtonContinue->Y - btnButtonContinue->Height / 2,
			btnButtonContinue->Width, btnButtonContinue->Height);
}
...
void __fastcall TiStatus::paintTo(Gdiplus::Graphics* aGraphics, EiUpdateType aUpdateType)
{
	...
	if (aUpdateType & updStatic)
	{
		...
		btnButtonContinue->paintTo(aGraphics);
	}
